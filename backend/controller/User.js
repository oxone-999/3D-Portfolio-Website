...
export const deleteModels = async(req,res) => {
    try {
        ...
        const item = user.models.filter((it) => it._id === id);

        await cloudinary.v2.uploader.destroy(item.image.public_id);

        user.models = user.models.filter((item) => item._id !== id);

        await user.save();
        
        res.status(200).json({
            success: true,
            message: "Deleted from Models",
        });
    } catch (error) {
        return res.status(400).json({
            success:false,
...
export const deleteProjects = async(req,res) => {
    try {
        ...
        const item = user.projects.filter((it) => it._id === id);
